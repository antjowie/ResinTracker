...
async function farmHandler(args, userId) {
    if (!args[0]) {
        ...
        day++;
        if (day === 0) {
            return ".... you can farm all books today (2) of your characters need to go to IDK we still dont have a location object";
        }
        ...
        team.forEach(name => {
            ...
            if ((day - 1) % 3 === genshinDict.talents[talent]) {
                if (!farmCharacters[talent]) {
                    farmCharacters[talent] = { "chars": [] };
                }
                farmCharacters[talent].chars.push(name);
            }
        });

        var message = [];

        for (const [key, value] of Object.entries(farmCharacters)) {
            message.push(`'${key}' can befarmed for ${value.chars} at IDK we dont have a location object \n`);
        };
        ...
        var user = await dbHandler.get("resin", userId);
        return `your team consists of ${user.team}`;
